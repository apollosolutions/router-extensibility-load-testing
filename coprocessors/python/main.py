...
JWT_SECRET = (
    environ.get("JWT_SECRET") if environ.get("JWT_SECRET") is not None else "apollo"
)
...
def handle_client_awareness():
    ...
    if payload["headers"].get("authentication") is None:
    ...
    try:
        jwt_payload = jwt.decode(token, key=JWT_SECRET, algorithms="HS256")
        client_name = (
            [jwt_payload.get("client_name")]
            if jwt_payload.get("client_name") is not None
            else ["coprocessor"]
        )
        client_version = (
            [jwt_payload.get("client_version")]
            if jwt_payload.get("client_version") is not None
            else ["loadtest"]
        )
        payload["headers"]["apollographql-client-name"] = client_name
        payload["headers"]["apollographql-client-version"] = client_version
